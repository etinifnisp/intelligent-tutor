...
import { authFetch, masteryColor, subjectColor, subjectIcon } from '../utils.jsx';
...
export default function DashboardPage({ user }) {
  ...
  function load() {
    Promise.all([
      authFetch('/stats/me').then(r => r.json()),
      authFetch('/chapters').then(r => r.json())
    ])
      ...
      <div className="dash-header">
        <div className="dash-stat">
          <div className="dash-stat-val" style={{ fontSize: 14, fontFamily: 'JetBrains Mono, monospace' }}>
            {user?.username?.substring(0, 20) || 'Student'}
          </div>
